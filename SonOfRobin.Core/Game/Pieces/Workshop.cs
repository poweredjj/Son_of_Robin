...
namespace SonOfRobin
{
    public class Workshop : BoardPiece
    {
        ...
        public Workshop(World world, Vector2 position, AnimData.PkgName animPackage, PieceTemplate.Name name, AllowedFields allowedFields, Dictionary<byte, int> maxMassBySize, MenuTemplate.Name craftMenuTemplate, string readableName, string description, Category category,
            ...
            base(world: world, position: position, animPackage: animPackage, animSize: animSize, animName: animName, blocksMovement: blocksMovement, minDistance: minDistance, maxDistance: maxDistance, name: name, destructionDelay: destructionDelay, allowedFields: allowedFields, floatsOnWater: floatsOnWater, maxMassBySize: maxMassBySize, generation: generation, canBePickedUp: false, yield: yield, maxHitPoints: maxHitPoints, fadeInAnim: fadeInAnim, readableName: readableName, description: description, isShownOnMiniMap: true, category: category, lightEngine: lightEngine)
        {
