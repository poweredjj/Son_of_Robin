...
namespace SonOfRobin
{
    public class Player : BoardPiece
    {
        ...
        public Player(World world, string id, AnimData.PkgName animPackage, PieceTemplate.Name name, AllowedTerrain allowedTerrain, string readableName, string description, State activeState, int strength, float speed, float maxStamina, float maxHitPoints, float maxFatigue, int craftLevel, byte invWidth, byte invHeight, byte toolbarWidth, byte toolbarHeight,
            byte animSize = 0, string animName = "default", bool ignoresCollisions = false, bool floatsOnWater = false, int generation = 0, Yield yield = null, int minDistance = 0, int maxDistance = 100, PieceSoundPack soundPack = null, int cookLevel = 1, int brewLevel = 1) :

            base(world: world, id: id, animPackage: animPackage, animSize: animSize, animName: animName, speed: speed, name: name, allowedTerrain: allowedTerrain, floatsOnWater: floatsOnWater, generation: generation, maxHitPoints: maxHitPoints, readableName: readableName, description: description, yield: yield, strength: strength, ignoresCollisions: ignoresCollisions, minDistance: minDistance, maxDistance: maxDistance, activeState: activeState, soundPack: soundPack, isAffectedByWind: false)
        {
