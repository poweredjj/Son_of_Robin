    ...
    public class Player : BoardPiece
    {
        ...
        public void ExpendEnergy(float energyAmount, bool addFatigue)
        {
            ...
            if (this.fedLevel > 0)
            {
                ...
                // SonOfRobinGame.messageLog.AddMessage(text: $"{SonOfRobinGame.CurrentUpdate} hungerVal {hungerVal}");
        ...
        private bool Walk(bool setOrientation = true, bool slowDownInWater = true, bool slowDownOnRocks = true)
        {
            ...
            if (analogWalk != Vector2.Zero)
            {
                ...
                // SonOfRobinGame.messageLog.AddMessage(text: $"{SonOfRobinGame.CurrentUpdate} vector {Math.Round(analogWalk.X, 1)},{Math.Round(analogWalk.Y, 1)} power {analogWalkTiltPower} speed {currentSpeed}");
            }
        ...
        public bool CheckForCookLevelUp()
        {
            ...
            if (levelUp)
            {
                ...
                if (levelMaster) imageList.Add(AnimData.GetCroppedFrameForPackage(AnimData.PkgName.Star).texture);
