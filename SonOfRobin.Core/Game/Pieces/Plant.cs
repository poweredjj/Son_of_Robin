...
namespace SonOfRobin
{
    ...
    public class Plant : BoardPiece
    {
        ...
        public Plant(World world, string id, AnimData.PkgName animPackage, PieceTemplate.Name name, AllowedTerrain allowedTerrain, string readableName, string description,
            ...
            byte animSize = 0, string animName = "default", float speed = 1, bool floatsOnWater = false, int staysAfterDeath = 800, int generation = 0, Yield yield = null, int maxHitPoints = 1, FruitEngine fruitEngine = null, AllowedDensity allowedDensity = null, LightEngine lightEngine = null, PieceSoundPack soundPack = null) :

            base(world: world, id: id, animPackage: animPackage, animSize: animSize, animName: animName, speed: speed, name: name, allowedTerrain: allowedTerrain, floatsOnWater: floatsOnWater, staysAfterDeath: staysAfterDeath, maxAge: maxAge, generation: generation, yield: yield, maxHitPoints: maxHitPoints, readableName: readableName, description: description, allowedDensity: allowedDensity, lightEngine: lightEngine, activeState: State.PlantGrowthAndReproduction, soundPack: soundPack)
        {
