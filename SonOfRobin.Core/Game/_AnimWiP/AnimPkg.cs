    ...
    public class AnimPkg
    {
        ...
        public static AnimPkg GetPackageForRpgMakerV1(AnimData.PkgName pkgName, string altasName, int colWidth, int colHeight, Vector2 gfxOffset, int setNoX, int setNoY, int animSize, float scale = 1f)
        {
            ...
            foreach (var kvp in yByDirection)
            {
                ...
                {
                    new AnimFrameNew(atlasName: altasName, layer: 1, cropRect: new Rectangle(x: width + setOffsetX, y: setOffsetY + directionOffsetY, width: width, height: height), duration: 0, scale: scale, gfxOffset: gfxOffset)
                };
