...
{
    public class VirtualPieceStorageOld : PieceStorage
    {
        ...
        public VirtualPieceStorageOld(BoardPiece storagePiece, World world, PieceStorage[,] storageArray) :
            base(width: 1, height: 1, world: world, storagePiece: storagePiece, storageType: storageArray[0, 0].storageType)
        {
            ...
            this.allowedPieceNames = this.CreateAllowedNames();
        ...
        public override bool AddPiece(BoardPiece piece, bool dropIfDoesNotFit = false, bool addMovement = false)
        {
            PieceStorage correctStorage = this.storageList[0]; // a storage must be used, even if neither can fit the piece

            foreach (PieceStorage pieceStorage in this.storageList)
            {
                StorageSlot slot = this.FindCorrectSlot(piece);
                if (slot != null)
                {
                    correctStorage = pieceStorage;
                    break;
                }
            }

            return correctStorage.AddPiece(piece: piece, dropIfDoesNotFit: dropIfDoesNotFit, addMovement: addMovement);
        }

        public override void Resize(byte newWidth, byte newHeight)
        {
            throw new ArgumentException("Virtual storage cannot be resized.");
        }
        ...
        private List<PieceTemplate.Name> CreateAllowedNames()
        {
        ...
        private List<PieceStorage> CreateStorageList()
        {
            var storageList = new List<PieceStorage>();
            ...
            for (int x = 0; x < this.arrayWidth; x++)
            {
                ...
                {
                    PieceStorage pieceStorage = this.storageArray[x, y];
                    if (pieceStorage != null) storageList.Add(pieceStorage);
                }
            }
